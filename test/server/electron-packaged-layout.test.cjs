...
test('resolveStageLayout：复制清单正确（最小 fixture，临时目录）', async () => {
  ...
  try {
    ...
    // 复制清单：routes 目录 + dist/domain 根级
    assert.ok(layout.copyDirs.some((c) => c.to.endsWith(path.join('server', 'routes'))), 'routes 复制');
    assert.ok(layout.copyRootDirs.some((c) => c.to.endsWith(path.join('dist', 'domain'))), 'dist/domain 根级复制');
    // 缺失目录登记
...
test('buildStageManifest：记录 dist/domain 产物（临时 stage）', async () => {
  const mod = await layoutModule();
...
test('最终 Resources 布局：../../dist 同构解析（临时 fixture，与生产同构）', () => {
  ...
  try {
    ...
    fs.mkdirSync(path.join(resources, 'server/routes'), { recursive: true });
    fs.mkdirSync(path.join(resources, 'dist/domain'), { recursive: true });
    // fixture 的 settings 片段与生产同构（../../dist 解析 policy）
    fs.writeFileSync(
      path.join(resources, 'server/routes/settings.js'),
      "module.exports = { policy: require('../../dist/domain/settings-policy.js') };\n",
    );
    fs.writeFileSync(
      path.join(resources, 'dist/domain/settings-policy.js'),
