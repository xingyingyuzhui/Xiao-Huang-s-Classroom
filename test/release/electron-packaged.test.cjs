 ...
 * 必须在 npm run pack:electron 之后执行；找不到 dist-electron/Resources
 * 时**必须失败**（不得 return、不得静默跳过）。
 */
...
test('electron-builder 真实产物：三关键文件存在且 settings route 可加载', () => {
  ...
  assert.ok(resources, '必须发现打包产物的 Resources 目录（无产物即失败）');
