...
test('balance-model: draftToPayload fills species from startEquation', async () => {
  const { draftToPayload, isPracticeFinished, buildEquation } = await import(
    '../../apps/web/src/chemistry/ai-classroom/balance-model.js'
  );
...
test('balance-model: isPracticeFinished uses equivalence not raw string', async () => {
  const { isPracticeFinished } = await import('../../apps/web/src/chemistry/ai-classroom/balance-model.js');
  const species = {
...
test('balance-model: buildEquation format accepted by check', async () => {
  const { buildEquation, isPracticeFinished } = await import('../../apps/web/src/chemistry/ai-classroom/balance-model.js');
  const species = {
...
test('balance-model: buildPracticeStepsFromEquations adds set_coef for target coefs', async () => {
  const { buildPracticeStepsFromEquations } = await import('../../apps/web/src/chemistry/ai-classroom/balance-model.js');
