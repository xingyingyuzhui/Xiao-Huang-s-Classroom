const { SUBJECT_TAB_CATALOG, READY_SUBJECT_IDS } = require('./tab-catalog.cjs');

const HUB_BRAND_TITLE = '小黄的教室';
...
function getSubjectTabMeta(subjectId) {
 ...
 */
function getSubjectCapabilities(subjectId) {
...
function defaultSubjectBrandTitle(subjectId) {
  const meta = getSubjectTabMeta(subjectId);
  return meta ? `小黄的${meta.name}教室` : HUB_BRAND_TITLE;
}

function getDefaultPageOptions(subjectId) {
  const meta = getSubjectTabMeta(subjectId);
  if (!meta) return [];
  return meta.tabs.map((t) => ({ id: t.id, label: t.label }));
}

function getDefaultTabId(subjectId) {
  const meta = getSubjectTabMeta(subjectId);
...
function isValidDefaultPage(subjectId, pageId) {
...
function createDefaultSubjectSettings() {
  const out = {};
...
function normalizeSubjectSettings(raw) {
...
function readSubjectAiFromMap(subjectSettings, subjectId = 'chemistry') {
