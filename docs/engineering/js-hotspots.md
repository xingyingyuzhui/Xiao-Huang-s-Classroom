...
| Graph 挂载/纯逻辑 | `apps/web/src/math/graph/graph-mount-controller.js`（743，hardMax 750）/ `graph-board-session.js`（143）/ `graph-document.js`（621）/ `graph-store.js`（514）/ `graph-record-validation.js`（519）/ `user-points.js`（572） | 生命周期/pure 边界（B2） | `node --test test/web/math-graph-board-session.test.cjs test/web/math-graph-mount-controller.test.cjs test/web/math-graph-document-renderer.test.cjs` + `(cd apps/web && npx vitest run ../../test/web/math-graph-document.vitest.ts ../../test/web/math-user-points.vitest.ts ../../test/web/math-graph-store.vitest.ts)` | pure 白名单成员（structure 测锁定）禁 jsxgraph/DOM；TS 化候选（graph-store/record-validation） |
