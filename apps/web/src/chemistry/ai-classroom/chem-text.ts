/**
 * 将题库中的少量 LaTeX 化学式转换成安全的展示 HTML（C3 后续：TS 权威源）。
 * 原始题干不做改写；本模块只用于浏览器渲染。纯字符串处理，无 DOM 依赖。
 */
...
function unwrapLatexCommandGroups(source: string, command: string): string {
...
function formatPlainText(value: unknown): string {
  ...
  return String(value).includes('\\') ? renderMath(String(value)) : escapeHtml(value);
}

/** 安全显示包含 $...$ 的题干或选项。 */
export function formatChemText(value: unknown): string {
...
export function formatChemOption(value: unknown): string {
 ...
 */
function formatQuizTable(tableHtml: string): string {
  ...
  const renderedRows = rows
    .map((row) => {
      const body = row[1] ?? '';
      const cells = [...body.matchAll(/<(td|th)\b[^>]*>([\s\S]*?)<\/\1\s*>/gi)];
      if (!cells.length) return '';
      return `<tr>${cells
        .map((cell) => {
          const tag = (cell[1] ?? 'td').toLowerCase();
          return `<${tag}>${formatChemText(cell[2] ?? '')}</${tag}>`;
        })
        .join('')}</tr>`;
    })
    .filter(Boolean);
...
function formatLatexArray(arraySource: string): string {
  ...
  return `<table class="quiz-table">${rows
    .map((row, rowIndex) => {
 ...
 */
export function formatChemPreview(value: unknown, limit = 60): string {
...
export function formatChemStem(value: unknown): string {
  ...
  while ((match = tablePattern.exec(source))) {
    ...
    output += full.startsWith('<') ? formatQuizTable(full) : formatLatexArray(full);
