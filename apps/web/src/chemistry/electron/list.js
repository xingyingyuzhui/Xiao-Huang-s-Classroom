...
import { settingsApi } from '../../shared/api/client.js';
import { patchCachedElectronOrder } from '../../shared/ui/settings.js';
...
async function loadOrderFromServer() {
  try {
    ...
    let raw = settings?.subjectSettings?.chemistry?.electronOrder;
        ...
        if (legacy) {
          ...
          if (Array.isArray(parsed) && parsed.length) {
            ...
            await settingsApi.update({
              subjectSettings: { chemistry: { electronOrder: normalized } },
            });
...
async function saveOrder() {
  try {
    ...
    await settingsApi.update({
      subjectSettings: { chemistry: { electronOrder: order } },
    });
