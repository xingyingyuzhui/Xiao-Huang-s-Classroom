/**
 * 高中化学知识地图 API（B2 第十三批：route TS 权威源）。
 *
 * db 注入模式：query 与 ensureQuizSchema（内部 require db）由组合根注入。
 * 知识分类目录（KNOWLEDGE_CATALOG）为确定性纯数据，随源迁移。
 */
/* eslint-disable @typescript-eslint/no-require-imports -- utils JS 引用经 tsup bundle */
import { Router, type Request, type Response } from 'express';

const { success, error } = require('../../utils/response') as {
  success: (res: Response, data?: unknown, message?: string) => Response;
  error: (res: Response, message?: string, status?: number) => Response;
};

/** 组合根注入：db 查询 + quiz schema 初始化（ensure-quiz-schema 权威源）。 */
export interface MasteryRouterDeps {
 ...
 */
function classifyQuestion(knowledge: unknown, stem: unknown): string {
  const knowledgeStr = String(knowledge || '').trim();
 ...
 * 正确率 >= 80% → 掌握
 */
function masteryLevel(total: number, correct: number): string {
...
/** 薄弱在前：起步 → 练习中 → 未开始 → 掌握 */
const LEVEL_ORDER: Record<string, number> = { beginner: 0, practicing: 1, unstarted: 2, mastered: 3 };
...
export function createMasteryRouter(deps: MasteryRouterDeps): Router {
  const { query, ensureQuizSchema } = deps;
  const router = Router();
  ...
  router.get('/', (_req: Request, res: Response) => {
    try {
      ...
      // 读取错题本
      const wrongItems = query(`SELECT knowledge, stem, dismissed FROM quiz_wrong_book`);
      ...
      for (const item of items) {
        ...
        if (correct) topicMap[topicId].correct += 1;
      }
      ...
      // 构建知识点列表
      const topics: Array<Record<string, unknown>> = KNOWLEDGE_CATALOG.map((cat) => {
      ...
      topics.sort(
        (a, b) =>
          (LEVEL_ORDER[String(a.level)] ?? 9) - (LEVEL_ORDER[String(b.level)] ?? 9),
      );
      ...
      const weakTopics = topics
        .filter(
          (t) =>
            (Number(t.total) > 0 && t.accuracy !== null && Number(t.accuracy) < 60) ||
            Number(t.wrong) > 0,
        )
        .slice(0, 5);
      ...
      if (totalQuestions === 0) {
        ...
        if (weak.length > 0) {
          suggestions.push(`以下知识点正确率较低，建议重点复习：${weak.map((t) => String(t.name)).join('、')}。`);
        }
        if (practicing.length > 0) {
          suggestions.push(`以下知识点正在巩固中，继续练习可以提升：${practicing.slice(0, 3).map((t) => String(t.name)).join('、')}。`);
        }
        if (totalWrong > 0) {
          suggestions.push(`错题本中有 ${totalWrong} 道未解决的错题，建议定期回顾。`);
        }
        if (unstarted.length > 0 && unstarted.length <= 5) {
          suggestions.push(`还有 ${unstarted.length} 个知识点未涉及：${unstarted.map((t) => String(t.name)).join('、')}。`);
        }
      ...
      error(res, err instanceof Error ? err.message : String(err) || '知识地图加载失败');
    }
  });
