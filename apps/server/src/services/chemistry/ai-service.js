const { callDeepSeekChat } = require('../ai/chat-service');
const { READY_SUBJECT_IDS } = require('@xiaohuang/subject-settings');
const { parseModelJson } = require('../ai/response-parser');
...
async function generateStoich(prompt, subjectId = 'chemistry') {
  const sid = resolveSubjectId(subjectId);
...
async function generateLab(prompt, subjectId = 'chemistry') {
  const sid = resolveSubjectId(subjectId);
...
async function generateBalanceStepTip({ equation, step = {}, subjectId = 'chemistry' } = {}) {
  const sid = resolveSubjectId(subjectId);
...
async function generateBalance(equation, subjectId = 'chemistry') {
  const sid = resolveSubjectId(subjectId);
