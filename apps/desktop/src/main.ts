/**
 * Electron 主进程（路线 A；C4：main.cjs → TS 权威源）。
 ...
 *
 * 打包布局（C4 单产物）：
 *   app.asar → electron/main.cjs（薄转发）→ dist/main.js（tsup 产物，
 *     bundle 含 startup-state-machine；asar 不再依赖 src/ 目录）
 *   resources/server → Express + public + node_modules（extraResources）
 *
 * 产物：tsup CJS dist/main.js（external electron，运行时由 Electron 提供）。
 */
import { app, BrowserWindow, shell, screen, Menu, dialog } from 'electron';
...
function getAppIcon(): string | undefined {
  const candidates = [
    ...
    path.join(__dirname, '..', 'web', 'public', 'brand-avatar.png'),
  ];
 ...
 * 用户改过缩放则以保存值为准
 */
function pickInitialZoom(display: Electron.Display): number {
...
function applyZoom(wc: Electron.WebContents, factor: number, persist = true): number {
...
function setupPageZoom(win: BrowserWindow): void {
...
async function startBackend(): Promise<{ server: { close(): void }; url: string }> {
...
function createWindow(url: string): Promise<void> {
